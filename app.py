...
def repo_card(repo: dict[str, Any]) -> None:
    description = repo.get("description") or "No description provided. The treasure may be hidden in the code."
    language = repo.get("language") or "Unknown"
    updated = (repo.get("pushed_at") or "").replace("T", " ").replace("Z", " UTC")
    st.markdown(
        f"""
        <div class="repo-card">
          <h3><a href="{repo.get('html_url')}" target="_blank">💎 {repo.get('full_name')}</a></h3>
          <p>{description}</p>
...
def render_charts(df: pd.DataFrame) -> None:
    ...
    with col2:
        ...
        st.scatter_chart(chart_df, x="stargazers_count", y="forks_count", size="treasure_score", color="#fbbf24")
